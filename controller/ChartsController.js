/**
 * Charts controller will handle the routes involving updating the Data Chart and Flot chart.
 */
...
exports.validateGetReportDataByExhchange = function getEarningsValidate(req,
		res, next) {
	console.log("Validating the getReportDataByExchange");
...
exports.getReportDataByExhchange = function(req, res) {
	...
	if (req.query.startDate != null && req.query.endDate != null) {
		startDateMoment = moment.utc(req.query.startDate);
		endDateMoment = moment.utc(req.query.endDate);
	} else {
		var d1 = moment.utc(new Date());
		endDateMoment = d1;

		var d2 = moment.utc(new Date());
		d2 = d2.subtract('days', 7);
		startDateMoment = d2;
	}

	// If the requested period is less that 4 days, show 7 days data.
	if (endDateMoment.diff(startDateMoment, 'days') < 4) {
		startDateMoment = moment(endDateMoment);
		startDateMoment = startDateMoment.subtract('days', 7);
	}
	...
	console.log('dfdsf');
	console.log("Report running ith parameters : ");
	console.log("StartDate : " + startDate);
	console.log("EndDate : " + endDate);

	// Call the DB for getting the AdExchnage records which fall in the given date range.
	AdExchangeModel.find().where('AD_TAG_NAME').equals(req.query.URL).where(
	...
	function prepareReportDataResponse(records) {
        console.log("*********records lenght "+ JSON.stringify(records));
		var response = {};
		...
		val["aaData"] = data;
		var chartData = prepareTimeSeriesData(dateTotalMap);
		
		chartData.totalEarnings = chartData.totalEarnings.toFixed(2);
        
		chartData.totalImpressions = chartData.totalImpressions.toFixed(2);
		chartData.totalEcpm = chartData.totalEcpm.toFixed(2);
